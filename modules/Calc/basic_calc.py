...
def Sub(v, base_state_dict, sub_state_dict, velocity):
    diff = base_state_dict - sub_state_dict
    return diff.to(v) * velocity
...
def StdSub(v, base_state_dict, sub_state_dict, velocity):
    ...
    return (
        ((base_state_dict - base_mean) / max(base_std, eps))
        - ((sub_state_dict - sub_mean) / max(sub_std, eps))
    ).to(v) * velocity
...
def PostAngle(v, processed_value, velocity):
    return torch.acos(
        torch.clamp(
            (v * processed_value).sum(dim=-1)
            / (torch.norm(v, dim=-1) * torch.norm(processed_value, dim=-1)),
            -1.0,
            1.0,
        )
    )
